...
HANDLER = logging.FileHandler(
    ...
    encoding="utf-8",
    mode=CONFIG.data.get("log_mode", "w"),
)
...
class Bot_Logging(commands.Cog):
    ...
    async def log_toggler(self, ctx: commands.Context, log_type, generic_name):
        ...
        if log_type == "traceback":
            await ctx.channel.send(f"{generic_name} is now: {humanize[state]}")
        else:
            await ctx.channel.send(f"{generic_name} logging is now: {humanize[state]}")
