...
path = "./bot/database/json/log_settings"

if not os.path.isfile(path):
    file = jthon.load(path)
    file.data = {
...
class Bot_Logging(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if self.bot.logging.data["log_command_error"]:
            ...
            LOGS.warning(
                f"Server_id: {ctx.message.guild.id} "
    ...
    async def log_toggler(self, ctx, log_type, generic_name):
        ...
        if log_type == "traceback":
            await ctx.channel.send(
                f"{generic_name} is now: {human[self.bot.logging.data[log_type]]}"
            )
        else:
            await ctx.channel.send(
                f"{generic_name} logging is now: {human[self.bot.logging.data[log_type]]}"
            )
